...
@section('header')
    <div class="page-header clearfix">
        <h3>
            <i class="fa fa-th-list" aria-hidden="true"></i> Client Adverts
            <a class="btn btn-sm btn-success pull-right" href="{{ route('clientadverts.create') }}"><i class="fa fa-plus-square-o" aria-hidden="true"></i>Create</a>
        </h3>
    </div>
        ...
        <div class="col-md-10 col-md-offset-1">
            ...
            @if($clientadverts->count())
                <table class="table table-condensed table-striped">
                    ...
                    <tbody><?php $i=1; ?>
                        @foreach($clientadverts as $value)
                            <tr>
                               ...
                               <td class="text-center"><h5>{{$value->issue->issue_no}}</h5></td>
                                ...
                                <td class="text-right">
                                    ...
                                    <a class="btn btn-sm btn-primary" href="{{ route('clientadverts.show', $value->id) }}">
                                        <i class="fa fa-eye" aria-hidden="true"></i> View
                                    </a>
                                    
                                    <a class="btn btn-sm btn-info" href="{{ route('clientadverts.edit', $value->id) }}"><i class="fa fa-pencil-square" aria-hidden="true"></i> Edit
                                    </a>
                                    @if($value->active_flag == 1 )
                                    <form action="{{ route('clientadverts.destroy', $value->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Delete? Are you sure?');">
                                        {{csrf_field()}}
                                    ...
                                    @else 
                                         <a class="btn btn-sm btn-success" href="{{ url('clientadverts/reactivate', $value->id) }}">
                                         Active ?
                                    </a>
                                    @endif
                                  ...
                                  <div class="modal-dialog">
                                    ...
                                    <div class="modal-content">
                                      ...
                                      <div class="modal-body">
                                          {!! Form::open(array('route'=>['clientadverts.meta',$value->id])) !!}
                                             @includeIf('./seoform')
                                          {!! Form::close() !!}
                                      </div>
