...
<div class="container">
    <div class="page-header clearfix">
        <h3>
            <i class="fa fa-th-list" aria-hidden="true"></i> CMS Pages
                        ...
                        @foreach($cmspages as $value)
                            <tr>
                                ...
                                <td class="text-right">
                                    ...
                                    <a class="btn btn-sm btn-primary" href="{{ route('cmspages.show', $value->id) }}">
                                        <i class="fa fa-eye" aria-hidden="true"></i> View
                                    </a>
                                    
                                    <a class="btn btn-sm btn-info" href="{{ route('cmspages.edit', $value->id) }}"><i class="fa fa-pencil-square" aria-hidden="true"></i> Edit
                                    </a>
                                    @if($value->active_flag == 1 )
                                    <form action="{{ route('cmspages.destroy', $value->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Delete? Are you sure?');">
                                        {{csrf_field()}}
                                    ...
                                    @else 
                                         <a class="btn btn-sm btn-success" href="{{ url('cmspages/reactivate', $value->id) }}">
                                         Active ?
                                    </a>
                                    @endif
                                  ...
                                  <div class="modal-dialog">
                                    ...
                                    <div class="modal-content">
                                      ...
                                      <div class="modal-body">
                                          {!! Form::open(array('route'=>['cmspages.meta',$value->id])) !!}
                                             @includeIf('./seoform')
                                          {!! Form::close() !!}
                                      </div>
