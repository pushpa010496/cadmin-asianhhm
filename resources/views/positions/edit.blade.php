...
@section('header')
    <div class="page-header">
        <h3><i class="fa fa-pencil-square" aria-hidden="true"></i> Pages / Edit #{{$position->id}}</h3>
    </div>
@endsection
            ...
            <form action="{{ route('positions.update', $position->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                 ...
                 <div class="form-group">
                    ...
                    {{ Form::input('text', 'position', old('position',$position->position), ['class' => 'form-control','required'=>'required']) }}
                </div>                            
                <div class="form-group">
                    <select name="active_flag" class="form-control" required="required">
                        <option value="">-- Select one --</option>
                        @if($position->active_flag == 1)
                        <option value="1" selected="selected">Active</option>
                        <option value="0">InActive</option>
                        @elseif($position->active_flag == 0)
                ...
                <div class="well well-sm">
                    ...
                    <a class="btn btn-sm btn-default pull-right" href="{{ route('positions.index') }}"><i class="fa fa-arrow-left" aria-hidden="true"></i>  Back</a>
                </div>
            </form>
